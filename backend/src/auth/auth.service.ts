...
export class AuthService {
  ...
  async validateUserJwt(payload: JwtPayload): Promise<SafeUser | null> {
    const jti = await this.cacheService.get(AuthService.JtiJwtCacheKey(payload.id));
  ...
  async signInUser(user: AuthenticatedRequest['user']) {
    const jwtPayload: JwtPayload = { email: user.email, id: user.id, jti: crypto.randomUUID() };
    const jwtExpiration = ms(this.config.get<string>('JWT_TOKEN_EXPIRES_IN')! as ms.StringValue);
    this.cacheService.set(AuthService.JtiJwtCacheKey(user.id), jwtPayload.jti, jwtExpiration);

    const refreshPayload: RefreshPayload = { id: user.id, jti: crypto.randomUUID() };
    const refreshExpiration = ms(this.config.get<string>('REFRESH_TOKEN_EXPIRES_IN')! as ms.StringValue);
    this.cacheService.set(AuthService.JtiRefreshCacheKey(user.id), refreshPayload.jti, refreshExpiration);
    return {
  ...
  async refreshJwt(jwtToken: string, refreshToken: string) {
    try {
      ...
      const jti = await this.cacheService.get(AuthService.JtiRefreshCacheKey(payload.id));
